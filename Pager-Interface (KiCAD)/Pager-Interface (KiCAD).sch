EESchema Schematic File Version 4
LIBS:Pager-Interface (KiCAD)-cache
EELAYER 26 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Connector:USB_B_Micro J1
U 1 1 603652A3
P 1450 2850
F 0 "J1" H 1505 3317 50  0000 C CNN
F 1 "USB_B_Micro" H 1505 3226 50  0000 C CNN
F 2 "Connector_USB:USB_Micro-B_GCT_USB3076-30-A" H 1600 2800 50  0001 C CNN
F 3 "~" H 1600 2800 50  0001 C CNN
F 4 "https://lcsc.com/product-detail/USB-Connectors_Shenzhen-Kinghelm-Elec-KH-MICRO5P-JZH_C962224.html" H 1450 2850 50  0001 C CNN "Shop"
	1    1450 2850
	1    0    0    -1  
$EndComp
Wire Wire Line
	1750 2650 1950 2650
Wire Wire Line
	1350 3250 1350 3550
Wire Wire Line
	1350 3550 1450 3550
Wire Wire Line
	1450 3550 1450 3250
Wire Wire Line
	1450 3550 1450 3650
Connection ~ 1450 3550
$Comp
L power:GND #PWR0101
U 1 1 603658FD
P 1450 3650
F 0 "#PWR0101" H 1450 3400 50  0001 C CNN
F 1 "GND" H 1455 3477 50  0000 C CNN
F 2 "" H 1450 3650 50  0001 C CNN
F 3 "" H 1450 3650 50  0001 C CNN
	1    1450 3650
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0102
U 1 1 60365973
P 1950 950
F 0 "#PWR0102" H 1950 800 50  0001 C CNN
F 1 "+5V" H 1965 1123 50  0000 C CNN
F 2 "" H 1950 950 50  0001 C CNN
F 3 "" H 1950 950 50  0001 C CNN
	1    1950 950 
	1    0    0    -1  
$EndComp
Wire Wire Line
	4950 4400 5200 4400
Wire Wire Line
	5200 4400 5200 4300
Wire Wire Line
	5200 4300 4950 4300
Wire Wire Line
	5200 4400 5200 4550
$Comp
L power:GND #PWR0103
U 1 1 60365D36
P 5200 4550
F 0 "#PWR0103" H 5200 4300 50  0001 C CNN
F 1 "GND" H 5205 4377 50  0000 C CNN
F 2 "" H 5200 4550 50  0001 C CNN
F 3 "" H 5200 4550 50  0001 C CNN
	1    5200 4550
	1    0    0    -1  
$EndComp
Text Label 10650 2500 0    50   ~ 0
RX0
Text Label 10650 2400 0    50   ~ 0
TX0
$Comp
L Device:C C4
U 1 1 60366ACE
P 4600 1100
F 0 "C4" H 4715 1146 50  0000 L CNN
F 1 "100nF" H 4715 1055 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 4638 950 50  0001 C CNN
F 3 "~" H 4600 1100 50  0001 C CNN
F 4 "C62912" H 4600 1100 50  0001 C CNN "LCSC"
	1    4600 1100
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0104
U 1 1 60366EE5
P 4600 1350
F 0 "#PWR0104" H 4600 1100 50  0001 C CNN
F 1 "GND" H 4605 1177 50  0000 C CNN
F 2 "" H 4600 1350 50  0001 C CNN
F 3 "" H 4600 1350 50  0001 C CNN
	1    4600 1350
	1    0    0    -1  
$EndComp
Wire Wire Line
	950  1250 950  1400
$Comp
L Device:C C1
U 1 1 60367798
P 950 1550
F 0 "C1" H 1065 1596 50  0000 L CNN
F 1 "100nF" H 1065 1505 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 988 1400 50  0001 C CNN
F 3 "~" H 950 1550 50  0001 C CNN
F 4 "C62912" H 950 1550 50  0001 C CNN "LCSC"
	1    950  1550
	1    0    0    -1  
$EndComp
Wire Wire Line
	950  1250 1500 1250
Wire Wire Line
	1500 1400 1500 1250
Connection ~ 1500 1250
Wire Wire Line
	1500 1250 1950 1250
Wire Wire Line
	950  1700 950  1850
Wire Wire Line
	1500 1700 1500 1850
$Comp
L power:GND #PWR0105
U 1 1 6036842B
P 950 1850
F 0 "#PWR0105" H 950 1600 50  0001 C CNN
F 1 "GND" H 955 1677 50  0000 C CNN
F 2 "" H 950 1850 50  0001 C CNN
F 3 "" H 950 1850 50  0001 C CNN
	1    950  1850
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0106
U 1 1 60368447
P 1500 1850
F 0 "#PWR0106" H 1500 1600 50  0001 C CNN
F 1 "GND" H 1505 1677 50  0000 C CNN
F 2 "" H 1500 1850 50  0001 C CNN
F 3 "" H 1500 1850 50  0001 C CNN
	1    1500 1850
	1    0    0    -1  
$EndComp
Wire Wire Line
	1950 950  1950 1250
Connection ~ 1950 1250
Connection ~ 1950 2650
$Comp
L Transistor_BJT:BC817 Q1
U 1 1 6036C595
P 7250 3900
F 0 "Q1" H 7441 3946 50  0000 L CNN
F 1 "BC817" H 7441 3855 50  0000 L CNN
F 2 "Package_TO_SOT_SMD:SOT-23" H 7450 3825 50  0001 L CIN
F 3 "http://www.fairchildsemi.com/ds/BC/BC817.pdf" H 7250 3900 50  0001 L CNN
F 4 "C8589" H 7250 3900 50  0001 C CNN "LCSC"
	1    7250 3900
	1    0    0    -1  
$EndComp
$Comp
L Transistor_BJT:BC817 Q2
U 1 1 6036C5CD
P 7250 4600
F 0 "Q2" H 7441 4554 50  0000 L CNN
F 1 "BC817" H 7441 4645 50  0000 L CNN
F 2 "Package_TO_SOT_SMD:SOT-23" H 7450 4525 50  0001 L CIN
F 3 "http://www.fairchildsemi.com/ds/BC/BC817.pdf" H 7250 4600 50  0001 L CNN
F 4 "C8589" H 7250 4600 50  0001 C CNN "LCSC"
	1    7250 4600
	1    0    0    1   
$EndComp
$Comp
L Device:R R1
U 1 1 6036D59F
P 6850 3900
F 0 "R1" V 6643 3900 50  0000 C CNN
F 1 "10k" V 6734 3900 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" V 6780 3900 50  0001 C CNN
F 3 "~" H 6850 3900 50  0001 C CNN
F 4 "C36231" V 6850 3900 50  0001 C CNN "LCSC"
	1    6850 3900
	0    1    1    0   
$EndComp
$Comp
L Device:R R2
U 1 1 6036D606
P 6850 4600
F 0 "R2" V 6643 4600 50  0000 C CNN
F 1 "10k" V 6734 4600 50  0000 C CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" V 6780 4600 50  0001 C CNN
F 3 "~" H 6850 4600 50  0001 C CNN
F 4 "C36231" V 6850 4600 50  0001 C CNN "LCSC"
	1    6850 4600
	0    1    1    0   
$EndComp
Wire Wire Line
	7000 3900 7050 3900
Wire Wire Line
	7000 4600 7050 4600
Wire Wire Line
	6100 4300 6600 4300
Wire Wire Line
	6600 4300 6600 3900
Wire Wire Line
	6600 3900 6700 3900
Wire Wire Line
	6700 4600 6550 4600
Wire Wire Line
	6550 4600 6550 4200
Wire Wire Line
	6600 4300 7350 4300
Wire Wire Line
	7350 4300 7350 4400
Connection ~ 6600 4300
Wire Wire Line
	6550 4200 7350 4200
Wire Wire Line
	7350 4200 7350 4100
Connection ~ 6550 4200
Wire Wire Line
	6550 4200 6550 3900
Wire Wire Line
	7350 4800 7350 4900
Wire Wire Line
	7350 3700 7350 3550
Text Label 7350 3550 0    50   ~ 0
EN
Text Label 7350 4900 3    50   ~ 0
IO0
$Comp
L ESP32:ESP32-WROOM U4
U 1 1 603711F2
P 9650 2950
F 0 "U4" H 9625 4337 60  0000 C CNN
F 1 "ESP32-WROOM" H 9625 4231 60  0000 C CNN
F 2 "ESP32:ESP32-WROOM" H 10000 4300 60  0001 C CNN
F 3 "" H 9200 3400 60  0001 C CNN
F 4 "C82899" H 9650 2950 50  0001 C CNN "LCSC"
	1    9650 2950
	1    0    0    -1  
$EndComp
Wire Wire Line
	8700 2450 8350 2450
Text Label 8350 2350 1    50   ~ 0
EN
Wire Wire Line
	10550 3400 10800 3400
Text Label 10800 3400 0    50   ~ 0
IO0
Wire Wire Line
	10650 2400 10550 2400
Wire Wire Line
	10650 2500 10550 2500
Text Label 3000 2600 2    50   ~ 0
TX0
Text Label 3000 2500 2    50   ~ 0
RX0
$Comp
L power:GND #PWR0107
U 1 1 60374DEC
P 7200 2450
F 0 "#PWR0107" H 7200 2200 50  0001 C CNN
F 1 "GND" V 7205 2322 50  0000 R CNN
F 2 "" H 7200 2450 50  0001 C CNN
F 3 "" H 7200 2450 50  0001 C CNN
	1    7200 2450
	0    1    1    0   
$EndComp
Wire Wire Line
	7200 2450 7350 2450
$Comp
L Device:R R3
U 1 1 6037582E
P 7750 2000
F 0 "R3" H 7820 2046 50  0000 L CNN
F 1 "10k" H 7820 1955 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" V 7680 2000 50  0001 C CNN
F 3 "~" H 7750 2000 50  0001 C CNN
F 4 "C17414" H 7750 2000 50  0001 C CNN "LCSC"
	1    7750 2000
	1    0    0    -1  
$EndComp
Wire Wire Line
	7750 2150 7750 2450
Wire Wire Line
	7750 2450 7650 2450
Wire Wire Line
	7750 1850 7750 1650
$Comp
L power:+3.3V #PWR0108
U 1 1 60377560
P 7750 1650
F 0 "#PWR0108" H 7750 1500 50  0001 C CNN
F 1 "+3.3V" H 7765 1823 50  0000 C CNN
F 2 "" H 7750 1650 50  0001 C CNN
F 3 "" H 7750 1650 50  0001 C CNN
	1    7750 1650
	1    0    0    -1  
$EndComp
Wire Wire Line
	8700 2350 8450 2350
Wire Wire Line
	8450 2350 8450 1500
$Comp
L power:+3.3V #PWR0109
U 1 1 60377F7F
P 8450 950
F 0 "#PWR0109" H 8450 800 50  0001 C CNN
F 1 "+3.3V" H 8465 1123 50  0000 C CNN
F 2 "" H 8450 950 50  0001 C CNN
F 3 "" H 8450 950 50  0001 C CNN
	1    8450 950 
	1    0    0    -1  
$EndComp
$Comp
L Device:C C8
U 1 1 6037A0D0
P 8900 1500
F 0 "C8" V 8648 1500 50  0000 C CNN
F 1 "100nF" V 8739 1500 50  0000 C CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 8938 1350 50  0001 C CNN
F 3 "~" H 8900 1500 50  0001 C CNN
F 4 "C62912" V 8900 1500 50  0001 C CNN "LCSC"
	1    8900 1500
	0    1    1    0   
$EndComp
Wire Wire Line
	8750 1100 8450 1100
Connection ~ 8450 1100
Wire Wire Line
	8450 1100 8450 950 
Wire Wire Line
	8750 1500 8450 1500
Connection ~ 8450 1500
Wire Wire Line
	8450 1500 8450 1100
Wire Wire Line
	9050 1500 9200 1500
Wire Wire Line
	9200 1500 9200 1300
Wire Wire Line
	9200 1100 9050 1100
Wire Wire Line
	9200 1300 9450 1300
Connection ~ 9200 1300
Wire Wire Line
	9200 1300 9200 1100
$Comp
L power:GND #PWR0110
U 1 1 6037E32B
P 9450 1300
F 0 "#PWR0110" H 9450 1050 50  0001 C CNN
F 1 "GND" V 9455 1172 50  0000 R CNN
F 2 "" H 9450 1300 50  0001 C CNN
F 3 "" H 9450 1300 50  0001 C CNN
	1    9450 1300
	0    -1   -1   0   
$EndComp
Wire Wire Line
	8700 3650 8400 3650
Wire Wire Line
	8400 3650 8400 3850
$Comp
L power:GND #PWR0111
U 1 1 6037F255
P 8400 3850
F 0 "#PWR0111" H 8400 3600 50  0001 C CNN
F 1 "GND" H 8405 3677 50  0000 C CNN
F 2 "" H 8400 3850 50  0001 C CNN
F 3 "" H 8400 3850 50  0001 C CNN
	1    8400 3850
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0112
U 1 1 6037F357
P 9050 4150
F 0 "#PWR0112" H 9050 3900 50  0001 C CNN
F 1 "GND" H 9055 3977 50  0000 C CNN
F 2 "" H 9050 4150 50  0001 C CNN
F 3 "" H 9050 4150 50  0001 C CNN
	1    9050 4150
	1    0    0    -1  
$EndComp
Wire Wire Line
	10550 3500 10750 3500
Wire Wire Line
	10750 3500 10750 3600
Wire Wire Line
	10750 3600 10550 3600
Wire Wire Line
	10750 3600 10750 3750
Connection ~ 10750 3600
$Comp
L power:GND #PWR0113
U 1 1 60382300
P 10750 3750
F 0 "#PWR0113" H 10750 3500 50  0001 C CNN
F 1 "GND" H 10755 3577 50  0000 C CNN
F 2 "" H 10750 3750 50  0001 C CNN
F 3 "" H 10750 3750 50  0001 C CNN
	1    10750 3750
	1    0    0    -1  
$EndComp
$Comp
L Jumper:SolderJumper_2_Open JP1
U 1 1 603826C5
P 8000 2450
F 0 "JP1" H 8000 2250 50  0000 C CNN
F 1 "EN Supply" H 8000 2350 50  0000 C CNN
F 2 "Jumper:SolderJumper-2_P1.3mm_Open_Pad1.0x1.5mm" H 8000 2450 50  0001 C CNN
F 3 "~" H 8000 2450 50  0001 C CNN
	1    8000 2450
	1    0    0    -1  
$EndComp
Wire Wire Line
	8350 2350 8350 2450
Connection ~ 8350 2450
Wire Wire Line
	8150 2450 8350 2450
Wire Wire Line
	7850 2450 7750 2450
Connection ~ 7750 2450
$Comp
L Regulator_Linear:AMS1117-3.3 U1
U 1 1 60389149
P 1950 4700
F 0 "U1" H 1950 4942 50  0000 C CNN
F 1 "AMS1117-3.3" H 1950 4851 50  0000 C CNN
F 2 "Package_TO_SOT_SMD:SOT-223-3_TabPin2" H 1950 4900 50  0001 C CNN
F 3 "http://www.advanced-monolithic.com/pdf/ds1117.pdf" H 2050 4450 50  0001 C CNN
F 4 "C6186" H 1950 4700 50  0001 C CNN "LCSC"
	1    1950 4700
	1    0    0    -1  
$EndComp
Wire Wire Line
	1300 4750 1300 4700
Wire Wire Line
	1300 4700 1650 4700
Wire Wire Line
	2250 4700 2550 4700
Wire Wire Line
	2550 4700 2550 4750
Wire Wire Line
	1950 5000 1950 5150
Wire Wire Line
	1300 5050 1300 5150
Wire Wire Line
	2550 5050 2550 5150
$Comp
L power:GND #PWR0114
U 1 1 60394963
P 1300 5150
F 0 "#PWR0114" H 1300 4900 50  0001 C CNN
F 1 "GND" H 1305 4977 50  0000 C CNN
F 2 "" H 1300 5150 50  0001 C CNN
F 3 "" H 1300 5150 50  0001 C CNN
	1    1300 5150
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0115
U 1 1 603949BB
P 1950 5150
F 0 "#PWR0115" H 1950 4900 50  0001 C CNN
F 1 "GND" H 1955 4977 50  0000 C CNN
F 2 "" H 1950 5150 50  0001 C CNN
F 3 "" H 1950 5150 50  0001 C CNN
	1    1950 5150
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0116
U 1 1 60394A13
P 2550 5150
F 0 "#PWR0116" H 2550 4900 50  0001 C CNN
F 1 "GND" H 2555 4977 50  0000 C CNN
F 2 "" H 2550 5150 50  0001 C CNN
F 3 "" H 2550 5150 50  0001 C CNN
	1    2550 5150
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0117
U 1 1 60394BB1
P 1300 4450
F 0 "#PWR0117" H 1300 4300 50  0001 C CNN
F 1 "+5V" H 1315 4623 50  0000 C CNN
F 2 "" H 1300 4450 50  0001 C CNN
F 3 "" H 1300 4450 50  0001 C CNN
	1    1300 4450
	1    0    0    -1  
$EndComp
Wire Wire Line
	1300 4700 1300 4450
Connection ~ 1300 4700
$Comp
L power:+3.3V #PWR0118
U 1 1 603965C3
P 2550 4450
F 0 "#PWR0118" H 2550 4300 50  0001 C CNN
F 1 "+3.3V" H 2565 4623 50  0000 C CNN
F 2 "" H 2550 4450 50  0001 C CNN
F 3 "" H 2550 4450 50  0001 C CNN
	1    2550 4450
	1    0    0    -1  
$EndComp
Wire Wire Line
	2550 4450 2550 4700
Connection ~ 2550 4700
$Comp
L si4432:SI4432-Module U3
U 1 1 6039ABF6
P 4750 5900
F 0 "U3" H 4750 6665 50  0000 C CNN
F 1 "SI4432-Module" H 4750 6574 50  0000 C CNN
F 2 "SI4432 Module:SI4432-Module" H 5400 6450 50  0001 C CNN
F 3 "" H 5400 6450 50  0001 C CNN
	1    4750 5900
	1    0    0    -1  
$EndComp
$Comp
L Connector:Conn_Coaxial J2
U 1 1 6039ADF5
P 5900 5400
F 0 "J2" H 6000 5376 50  0000 L CNN
F 1 "SMA ANT" H 6000 5285 50  0000 L CNN
F 2 "Connector_Coaxial:SMA_Amphenol_132289_EdgeMount" H 5900 5400 50  0001 C CNN
F 3 " ~" H 5900 5400 50  0001 C CNN
F 4 "C976230" H 5900 5400 50  0001 C CNN "Shop"
	1    5900 5400
	1    0    0    -1  
$EndComp
Wire Wire Line
	5250 5400 5700 5400
Wire Wire Line
	5900 5600 5250 5600
Wire Wire Line
	5250 5600 5250 5500
Wire Wire Line
	4250 5400 4150 5400
Wire Wire Line
	4150 5400 4150 5250
Wire Wire Line
	4250 6500 4100 6500
Wire Wire Line
	4100 6500 4100 6650
$Comp
L power:GND #PWR0119
U 1 1 603A1EF6
P 5250 5700
F 0 "#PWR0119" H 5250 5450 50  0001 C CNN
F 1 "GND" H 5255 5527 50  0000 C CNN
F 2 "" H 5250 5700 50  0001 C CNN
F 3 "" H 5250 5700 50  0001 C CNN
	1    5250 5700
	1    0    0    -1  
$EndComp
Wire Wire Line
	5250 5600 5250 5700
Connection ~ 5250 5600
$Comp
L power:GND #PWR0120
U 1 1 603A3E00
P 4150 5250
F 0 "#PWR0120" H 4150 5000 50  0001 C CNN
F 1 "GND" H 4155 5077 50  0000 C CNN
F 2 "" H 4150 5250 50  0001 C CNN
F 3 "" H 4150 5250 50  0001 C CNN
	1    4150 5250
	-1   0    0    1   
$EndComp
$Comp
L power:GND #PWR0121
U 1 1 603A3EA1
P 4100 6650
F 0 "#PWR0121" H 4100 6400 50  0001 C CNN
F 1 "GND" H 4105 6477 50  0000 C CNN
F 2 "" H 4100 6650 50  0001 C CNN
F 3 "" H 4100 6650 50  0001 C CNN
	1    4100 6650
	1    0    0    -1  
$EndComp
Wire Wire Line
	4250 5800 3900 5800
Wire Wire Line
	3900 5800 3900 5250
$Comp
L power:+3.3V #PWR0122
U 1 1 603A5FDC
P 3900 5250
F 0 "#PWR0122" H 3900 5100 50  0001 C CNN
F 1 "+3.3V" H 3915 5423 50  0000 C CNN
F 2 "" H 3900 5250 50  0001 C CNN
F 3 "" H 3900 5250 50  0001 C CNN
	1    3900 5250
	1    0    0    -1  
$EndComp
Wire Wire Line
	4250 5500 3650 5500
Wire Wire Line
	4250 5600 3650 5600
Wire Wire Line
	4250 5700 3650 5700
Text Label 3650 5500 2    50   ~ 0
RX_Data
Text Label 3650 5600 2    50   ~ 0
RX_Clock
Text Label 3650 5700 2    50   ~ 0
RSSI
Wire Wire Line
	4250 5900 3650 5900
Wire Wire Line
	4250 6000 3650 6000
Wire Wire Line
	4250 6100 3650 6100
Wire Wire Line
	4250 6200 3650 6200
Text Label 3650 5900 2    50   ~ 0
MISO
Text Label 3650 6000 2    50   ~ 0
MOSI
Text Label 3650 6100 2    50   ~ 0
CLK
Text Label 3650 6200 2    50   ~ 0
CS
Wire Wire Line
	4250 6300 3650 6300
Wire Wire Line
	4250 6400 3650 6400
Text Label 3650 6300 2    50   ~ 0
nIRQ
Text Label 3650 6400 2    50   ~ 0
SDN
Text Label 10650 2900 0    50   ~ 0
RX_Clock
Wire Wire Line
	10550 2900 10650 2900
Text Label 10650 2800 0    50   ~ 0
RX_Data
Wire Wire Line
	10550 2800 10650 2800
Text Label 10650 2600 0    50   ~ 0
RSSI
Wire Wire Line
	10550 2600 10650 2600
Text Label 10650 3000 0    50   ~ 0
nIRQ
Wire Wire Line
	10650 3000 10550 3000
Text Label 10650 3200 0    50   ~ 0
SDN
Wire Wire Line
	10550 3200 10650 3200
Text Label 8550 3550 2    50   ~ 0
MISO
Wire Wire Line
	8700 3550 8550 3550
Text Label 9300 4100 3    50   ~ 0
MOSI
Wire Wire Line
	9200 4000 9200 4100
Wire Wire Line
	9200 4100 9050 4100
Wire Wire Line
	9050 4100 9050 4150
Wire Wire Line
	9300 4000 9300 4100
Text Label 8550 3450 2    50   ~ 0
CLK
Wire Wire Line
	8700 3450 8550 3450
Text Label 10000 4100 3    50   ~ 0
CS
Wire Wire Line
	10000 4000 10000 4100
$Comp
L Device:C C6
U 1 1 603E1A86
P 7500 2450
F 0 "C6" V 7248 2450 50  0000 C CNN
F 1 "1uF" V 7339 2450 50  0000 C CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 7538 2300 50  0001 C CNN
F 3 "~" H 7500 2450 50  0001 C CNN
F 4 "C282736" V 7500 2450 50  0001 C CNN "LCSC"
	1    7500 2450
	0    1    1    0   
$EndComp
$Comp
L Device:C C7
U 1 1 603E1C23
P 8900 1100
F 0 "C7" V 8648 1100 50  0000 C CNN
F 1 "10uF" V 8739 1100 50  0000 C CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 8938 950 50  0001 C CNN
F 3 "~" H 8900 1100 50  0001 C CNN
F 4 "C15850" V 8900 1100 50  0001 C CNN "LCSC"
	1    8900 1100
	0    1    1    0   
$EndComp
$Comp
L Device:C C2
U 1 1 603E1E7F
P 1300 4900
F 0 "C2" V 1048 4900 50  0000 C CNN
F 1 "10uF" V 1139 4900 50  0000 C CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 1338 4750 50  0001 C CNN
F 3 "~" H 1300 4900 50  0001 C CNN
F 4 "C15850" V 1300 4900 50  0001 C CNN "LCSC"
	1    1300 4900
	-1   0    0    1   
$EndComp
$Comp
L Device:C C5
U 1 1 603E20BD
P 2550 4900
F 0 "C5" V 2298 4900 50  0000 C CNN
F 1 "10uF" V 2389 4900 50  0000 C CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 2588 4750 50  0001 C CNN
F 3 "~" H 2550 4900 50  0001 C CNN
F 4 "C15850" V 2550 4900 50  0001 C CNN "LCSC"
	1    2550 4900
	-1   0    0    1   
$EndComp
$Comp
L Device:C C3
U 1 1 603E22DF
P 1500 1550
F 0 "C3" H 1385 1504 50  0000 R CNN
F 1 "4.7uF" H 1385 1595 50  0000 R CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 1538 1400 50  0001 C CNN
F 3 "~" H 1500 1550 50  0001 C CNN
F 4 "C1746" H 1500 1550 50  0001 C CNN "LCSC"
	1    1500 1550
	-1   0    0    1   
$EndComp
$Comp
L CP2102N-A01-GQFN28:CP2102N-A01-GQFN28 U2
U 1 1 60381793
P 4050 3200
F 0 "U2" H 4050 4567 50  0000 C CNN
F 1 "CP2102N-A01-GQFN28" H 4050 4476 50  0000 C CNN
F 2 "CP2102:CP2102-GQFN28" H 4050 3200 50  0001 L BNN
F 3 "" H 4050 3200 50  0001 L BNN
F 4 "IPC7351B" H 4050 3200 50  0001 L BNN "STANDARD"
F 5 "1.2" H 4050 3200 50  0001 L BNN "PARTREV"
F 6 "Silicon Labs" H 4050 3200 50  0001 L BNN "MANUFACTURER"
F 7 "C6568" H 4050 3200 50  0001 C CNN "LCSC"
	1    4050 3200
	1    0    0    -1  
$EndComp
Wire Wire Line
	4950 2100 5100 2100
Wire Wire Line
	5100 2100 5100 750 
Wire Wire Line
	5100 750  4600 750 
Wire Wire Line
	4600 750  4600 950 
Wire Wire Line
	6100 3900 6550 3900
Wire Wire Line
	4600 1250 4600 1350
Connection ~ 5200 4400
Text Label 6100 3900 2    50   ~ 0
RST
Text Label 3000 2700 2    50   ~ 0
RST
Wire Wire Line
	3000 2700 3150 2700
Text Label 6100 4300 2    50   ~ 0
DTR
Text Label 3000 3000 2    50   ~ 0
DTR
Wire Wire Line
	3000 3000 3150 3000
Wire Wire Line
	1950 1250 1950 2500
Text Label 2100 2500 0    50   ~ 0
REGIN
Wire Wire Line
	1950 2500 2100 2500
Text Label 5100 2400 0    50   ~ 0
REGIN
Wire Wire Line
	5100 2400 4950 2400
Text Label 2100 2650 0    50   ~ 0
VBUS
Wire Wire Line
	2100 2650 1950 2650
Text Label 5100 2200 0    50   ~ 0
VBUS
Wire Wire Line
	5100 2200 4950 2200
Text Label 2100 2850 0    50   ~ 0
D+
Wire Wire Line
	2100 2850 1750 2850
Text Label 3000 3300 2    50   ~ 0
D+
Wire Wire Line
	3000 3300 3150 3300
Wire Wire Line
	1750 2950 2100 2950
Text Label 2100 2950 0    50   ~ 0
D-
Text Label 3000 3400 2    50   ~ 0
D-
Wire Wire Line
	3000 3400 3150 3400
Connection ~ 1950 2500
Wire Wire Line
	1950 2500 1950 2650
Wire Wire Line
	3000 2600 3150 2600
Wire Wire Line
	3000 2500 3150 2500
$Comp
L Connector_Generic:Conn_01x05 J3
U 1 1 60428612
P 6200 7050
F 0 "J3" H 6280 7092 50  0000 L CNN
F 1 "Debug SI4432" H 6280 7001 50  0000 L CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x05_P2.54mm_Vertical" H 6200 7050 50  0001 C CNN
F 3 "~" H 6200 7050 50  0001 C CNN
	1    6200 7050
	1    0    0    -1  
$EndComp
Text Label 5850 6850 2    50   ~ 0
RX_Data
Text Label 5850 6950 2    50   ~ 0
RX_Clock
Text Label 5850 7050 2    50   ~ 0
RSSI
Text Label 5850 7150 2    50   ~ 0
nIRQ
$Comp
L power:GND #PWR0123
U 1 1 60438814
P 5850 7400
F 0 "#PWR0123" H 5850 7150 50  0001 C CNN
F 1 "GND" H 5855 7227 50  0000 C CNN
F 2 "" H 5850 7400 50  0001 C CNN
F 3 "" H 5850 7400 50  0001 C CNN
	1    5850 7400
	1    0    0    -1  
$EndComp
Wire Wire Line
	5850 7400 5850 7250
Wire Wire Line
	5850 7250 6000 7250
Wire Wire Line
	6000 7150 5850 7150
Wire Wire Line
	6000 6950 5850 6950
Wire Wire Line
	5850 7050 6000 7050
Wire Wire Line
	5850 6850 6000 6850
$Comp
L Connector_Generic:Conn_01x03 J4
U 1 1 60451FB8
P 2950 1250
F 0 "J4" H 3030 1292 50  0000 L CNN
F 1 "UART" H 3030 1201 50  0000 L CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x03_P2.54mm_Vertical" H 2950 1250 50  0001 C CNN
F 3 "~" H 2950 1250 50  0001 C CNN
	1    2950 1250
	1    0    0    -1  
$EndComp
Text Label 2600 1150 2    50   ~ 0
RX0
Wire Wire Line
	2600 1150 2750 1150
Text Label 2600 1250 2    50   ~ 0
TX0
Wire Wire Line
	2600 1250 2750 1250
$Comp
L power:GND #PWR0124
U 1 1 6045B607
P 2650 1550
F 0 "#PWR0124" H 2650 1300 50  0001 C CNN
F 1 "GND" H 2655 1377 50  0000 C CNN
F 2 "" H 2650 1550 50  0001 C CNN
F 3 "" H 2650 1550 50  0001 C CNN
	1    2650 1550
	1    0    0    -1  
$EndComp
Wire Wire Line
	2650 1550 2650 1350
Wire Wire Line
	2650 1350 2750 1350
$EndSCHEMATC
